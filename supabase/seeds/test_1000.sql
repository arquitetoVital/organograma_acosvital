-- ============================================================
-- DADOS DE TESTE — ~1000 funcionários
--
-- Diferenças vs. 700: n_lider=2, n_anl=3, n_asst=4, n_apr=1
-- Para remover: DELETE FROM organograma.org_nodes WHERE id LIKE 'tst1000-%';
-- ============================================================

DELETE FROM organograma.org_nodes WHERE id LIKE 'tst1000-%';
...
DECLARE
  ...
  n_sup       int  := 1;
  n_lider     int  := 2;
  n_anl       int  := 3;   -- ← 3 analistas por líder
  n_asst      int  := 4;   -- ← 4 assistentes por líder
  n_apr       int  := 1;
  n_spec_asst int  := 60;
  n_spec_apr  int  := 30;
...
BEGIN
  ...
  RAISE NOTICE 'tst1000 — % pessoas inseridas (total com estrutura: %)', seq, seq + 24;
END $$;
