-- ============================================================
-- DADOS DE TESTE — ~400 funcionários
--
-- Estrutura:
--   • 1 diretoria casal (central)
--   • 5 Gerentes Gerais
--   • 18 Setores  (17 completos + Serviços Gerais só com aux/apr)
--   • 1 Diretoria por setor (sectorDirectorOf)
--   • 2 Sub-setores por setor
--   • Hierarquia: Gerente → Coord → Sup → Líder → Analista/Assistente/Aprendiz
--
-- Para remover: DELETE FROM organograma.org_nodes WHERE id LIKE 'tst400-%';
-- ============================================================

DELETE FROM organograma.org_nodes WHERE id LIKE 'tst400-%';

DO $$
DECLARE
  pfx         text := 'tst400';
  n_sup       int  := 1;   -- supervisores por coordenador
  n_lider     int  := 1;   -- líderes por supervisor
  n_anl       int  := 2;   -- analistas por líder
  n_asst      int  := 2;   -- assistentes por líder
  n_apr       int  := 1;   -- aprendizes por assistente
  n_spec_asst int  := 20;  -- auxiliares em Serviços Gerais
  n_spec_apr  int  := 10;  -- aprendizes em Serviços Gerais
...
BEGIN
  ...
  -- Setores + Diretorias de setor + Sub-setores
  FOR s IN 1..18 LOOP
    INSERT INTO organograma.org_nodes VALUES (
  ...
  -- Pessoas nos setores regulares (1–17)
  FOR s IN 1..17 LOOP
    seq := seq + 1;
    INSERT INTO organograma.org_nodes VALUES (
    ...
    FOR ss IN 1..2 LOOP
      ...
      FOR ksup IN 1..n_sup LOOP
        ...
        END LOOP; -- líderes
      END LOOP; -- supervisores
    END LOOP; -- sub-setores
  END LOOP; -- setores

  -- Setor especial 18 (Serviços Gerais): apenas auxiliares e aprendizes
  ...
  RAISE NOTICE 'tst400 — % pessoas inseridas (total com estrutura: %)', seq, seq + 24;
END $$;
