-- ============================================================
-- DADOS DE TESTE — ~700 funcionários
--
-- Diferenças vs. 400: n_lider=2, n_asst=3, setor especial maior
-- Para remover: DELETE FROM organograma.org_nodes WHERE id LIKE 'tst700-%';
-- ============================================================

DELETE FROM organograma.org_nodes WHERE id LIKE 'tst700-%';

DO $$
DECLARE
  pfx         text := 'tst700';
  n_sup       int  := 1;
  n_lider     int  := 2;   -- ← 2 líderes por supervisor
  n_anl       int  := 2;
  n_asst      int  := 3;   -- ← 3 assistentes por líder
  n_apr       int  := 1;
  n_spec_asst int  := 40;
  n_spec_apr  int  := 20;
...
BEGIN
  ...
  RAISE NOTICE 'tst700 — % pessoas inseridas (total com estrutura: %)', seq, seq + 24;
END $$;
